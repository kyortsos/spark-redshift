...
private[redshift] class RedshiftWriter(
  ...
  private def unloadData(
    ...
    (tempFormat match {
        ...
        case "CSV" => writer.format("com.databricks.spark.csv").option("nullValue", nullString)
    }).save(tempDir)
  ...
  /**
   * Write a DataFrame to a Redshift table, using S3 and Avro, JSON, or CSV serialization
   */
  def saveToRedshift(
      sqlContext: SQLContext,
      data: DataFrame,
